...
class Complex:
    ...
    def __str__(self):
        if self.imaginary == 0: # b=0
            return f'{self.real}'
        elif self.real == 0: # b!=0 and a = 0
            if self.imaginary > 0:
...
def prune(t, n):
    ...
    if len(t.branches) > n:
        ...
        for _ in range(len(t.branches) - n):
            t.branches.pop(labels.index(max(labels)))
            labels.pop(labels.index(max(labels)))
