...
def accumulate(combiner, base, n, f):
    ...
    "*** YOUR CODE HERE ***"
    if n > 0 :
        re = combiner(base,f(1))
        for i in range(1,n):
            re = combiner(re,f(i+1))
        return re 
    else:
        return base
...
def summation_using_accumulate(n, f):
    ...
    True
    """
    return accumulate(add,f(n),n-1,f)
...
def product_using_accumulate(n, f):
    ...
    True
    """
    return accumulate(mul,f(n),n-1,f)
...
def one(f):
    ...
    "*** YOUR CODE HERE ***"
    return lambda f:lambda x: f(zero)

def two(f):
    """Church numeral 2: same as successor(successor(zero))"""
    "*** YOUR CODE HERE ***"
    return lambda f:lambda x: f(one)
